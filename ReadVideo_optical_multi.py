...
if Flag_training_or_test == True:
    ...
    dir = ".\Data\\training_optical_multi\\"
else:
    globFilters = ['./Data/Recorded/*.*']
    dir = ".\Data\\testing_optical_multi\\"
...
for k in range(len(globFilters)):
    ...
    for i in range(len(filelist)):  #len(filelist)
        ...
        step = int(lenOfVideo / 11)
        mid =  int(lenOfVideo / 2)
        cur = 0
        # Build 10 optical flow images
        for n in range(10):
            cur = cur + step - 1
            print(str(cur) + "-" + str(lenOfVideo))
            ...
            if ret == True:
                ...
                fileName = fileName_ori + "_" + str(n) + ".jpg"
                fileName_of = fileName_ori + "_of_" + str(n) + ".jpg"
                path = dir + fileName
